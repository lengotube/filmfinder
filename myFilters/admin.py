...
class IsAdminC(BoundFilter):
    async def check(self, call: types.CallbackQuery):
        return call.from_user.id in admin_id

class IsAdminCAndChenneger_swich_player(BoundFilter):
    async def check(self, call: types.CallbackQuery):
        return call.from_user.id in admin_id and call.data[0:28] == 'chenneger_swich_player_admin'

class IsAdminCAndChenneger_kbname_player_admin(BoundFilter):
    async def check(self, call: types.CallbackQuery):
        return call.from_user.id in admin_id and call.data[0:29] == 'chenneger_kbname_player_admin'